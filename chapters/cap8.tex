...
O \textit{Random Forest} demonstrou ser a abordagem mais equilibrada entre as supervisionadas. Superou a DNN em macro F1 em todos os cenários avaliados — 0,86 contra 0,67 no Baseline e 0,97 contra 0,87 com o \textit{LycoS-IDS2017} — e realizou a inferência de 848.363 amostras em 3,35 segundos, contra 17,33 segundos da DNN, uma diferença de 5 vezes. Sua degradação em cenários de generalização foi mais contida e localizada: ao enfrentar o \textit{PortScan} desconhecido, manteve acurácia de 0,83 com impacto concentrado apenas na classe ausente, enquanto a DNN sofreu falhas em múltiplas classes não relacionadas. Para ambientes com dados rotulados disponíveis, restrições de hardware ou requisitos de baixa latência, o \textit{Random Forest} é a escolha mais segura.

A DNN, na configuração investigada, não se mostrou competitiva frente ao \textit{Random Forest} em nenhum dos cenários. No entanto, o dado mais relevante é a assimetria de ganho com a melhoria dos dados: o \textit{LycoS-IDS2017} elevou a macro F1 da DNN em 0,20 pontos, contra 0,11 do \textit{Random Forest}, indicando que a rede neural é mais sensível à qualidade do \textit{dataset} e tem potencial mais alto quando treinada sobre dados confiáveis. A redução de dimensionalidade por MDI não ajudou a DNN em nenhum cenário, pois a seleção de \textit{features} por impureza de Gini favorece as classes majoritárias e descarta atributos críticos para a discriminação de ataques raros. Portanto, o uso de MDI como pré-processamento para DNN em \textit{datasets} desbalanceados deve ser evitado.
